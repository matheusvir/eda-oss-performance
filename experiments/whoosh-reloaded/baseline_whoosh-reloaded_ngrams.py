"""Baseline benchmark for wildcard lookup without ngram acceleration.

This benchmark measures wildcard queries that do not have a literal prefix
(e.g. ``*ados`` and ``*ritmo*``). In vanilla behavior, these queries trigger
a linear scan over the full field lexicon.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Baseline: wildcard lookup performance using lexicon linear scan "
            "(no ngram wildcard optimization)"
        )
    )
...
def main() -> int:
    ...
    index_dir = tempfile.mkdtemp(prefix="whoosh_ngrams_baseline_")
    try:
